...
const Navbar = () => {
  ...
  const {user, setShowLogin, logout, credit, creditsLoading} = useContext(AppContext);
      ...
      <div>
        {user ? (
          <div className="flex items-center gap-2 sm:gap-3">
            <button className="flex items-center gap-2 bg-blue-100 px-4 sm:px-6 py-1.5 sm:py-3 rounded-full hover:scale-105 transition-all duration-700" onClick={()=>{navigate('/buycredit')}}>
              ...
              <p className="text-xs sm:text-sm font-medium">
                Credit left : {creditsLoading ? "..." : credit}
              </p>
            </button>
          ...
          <div className="flex items-center gap-2 sm:gap-5">
            {creditsLoading ? (
              <Loader label="Checking session" size="sm" inline />
            ) : (
              <p
